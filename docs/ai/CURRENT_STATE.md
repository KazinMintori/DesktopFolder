...
- **`VERIFIED`**: **Virtual Shell Members**: Recycle Bin, This PC, and Network can participate in mixed or Shell-only collections. They persist a stable `KNOWNFOLDERID` identity and original visibility, resolve PIDLs only at runtime, and bypass filesystem APIs (`DirectDesktopFolders.cs:55-65`, `408-605`).
...
- **`VERIFIED`**: **Other Virtual System Icons Unsupported**: Recycle Bin, This PC, and Network are supported explicitly; other non-filesystem namespace icons (for example Control Panel) remain excluded by `ExplorerDesktop.Scan()` (`DirectDesktopFolders.cs:851-883`).
...
- **`VERIFIED`**: **Multi-Monitor Awareness**: Popup placement calculations (`CalculateAnchoredLocation`, `FindOpenLocation`) and Desktop fallback placement (`PreferredDesktopPoint`) are monitor-aware across multi-display setups while maintaining single-monitor bounds safety ([DirectDesktopFolders.cs:1754-1773](file:///C:/Users/Ai/.gemini/antigravity/worktrees/DesktopFolder/create_project_map_documentation/DirectDesktopFolders.cs#L1754-L1773), [3446-3453](file:///C:/Users/Ai/.gemini/antigravity/worktrees/DesktopFolder/create_project_map_documentation/DirectDesktopFolders.cs#L3446-L3453)).
- **`VERIFIED`**: **Localization (i18n)**: Centralized `Loc` helper with dual loading (external JSON in `Resources/strings.{lang}.json` with embedded assembly manifest fallback). Supports System culture (`CultureInfo.CurrentUICulture`), English (`en`), and Vietnamese (`vi`), with language selection in Settings, live tray/UI updates, and fallback to English for missing keys.
